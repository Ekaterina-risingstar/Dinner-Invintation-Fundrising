...
const App: React.FC = () => {
  ...
  const isFormValid = formData.fullName && formData.attendance && formData.mealPreference && formData.hasConfirmedPayment;
  ...
  const paymentReferenceText = `${formData.fullName || '{Full Name}'} — Fundraising Dinner`;

  // Proprietary link for mobile "click to pay" functionality (might not be universally supported)
  const paymentLink = `iban-qr://pay?name=${encodeURIComponent(RECIPIENT)}&iban=${IBAN}&bic=${BIC}&amount=${totalAmount.toFixed(2)}&reason=${encodeURIComponent(paymentReferenceText.substring(0, 140))}`;
          ...
          <div className="p-8 md:p-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="border-t border-gray-200 pt-8">
                ...
                <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-6 flex flex-col md:flex-row gap-6">
                  <div className="flex-grow space-y-2 text-sm text-gray-700">
                      ...
                      <p className="mt-2"><strong>Amount:</strong> <span className="font-bold text-indigo-600 text-lg">€{totalAmount.toFixed(2)}</span></p>
                  ...
                  <div className="flex-shrink-0 text-center">
                    <p className="text-sm font-bold text-gray-700 mb-2">Scan to Pay</p>
                    <div className="bg-white p-2 border rounded-md inline-block shadow-sm">
                     ...
                     <a href={paymentLink} className="mt-4 inline-block text-indigo-600 hover:text-indigo-800 text-sm font-semibold">
                      👉 Click here to pay
                      <span className="block text-xs text-gray-500">(Opens banking app on mobile)</span>
                    </a>
                  </div>
                </div>
              </div>
